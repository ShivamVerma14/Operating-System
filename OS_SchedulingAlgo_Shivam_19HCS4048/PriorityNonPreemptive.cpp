...
class Process
{
    ...
    public:
        // A constructor
        Process() : processNo(0), arrivalTime(0), burstTime(0), priority(0), waitingTime(0), turnAroundTime(0), status(0) {}
...
void schedule(Process array[], int no)
{
    ...
    while(completed != no)
    {
        ...
        for(int iterate = 0; iterate < no; ++iterate)
            if(array[iterate].arrivalTime <= currentTime && array[iterate].status == 0)
            {
        ...
        if(index != -1)
        {
            currentTime += array[index].burstTime;
            completed++;
...
int main()
{
    ...
    for(int index = 0; index < noOfProcess; ++index)
    {
        ...
        processes[index].takeInformation(index);
    }
    system("cls");
